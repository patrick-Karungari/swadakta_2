 ...
 * Class AuthModel
 *
 * @property \App\Libraries\Auth $ion_auth The Ion_auth library
 */
class AuthModel
{
	/**
	...
	public function getUserByActivationCode(string $userCode)
	{
		...
		$user = $this->where('activation_selector', $token->selector)->users();
	...
	public function activate($id, string $code=''): bool
	{
		...
		if (!$code || ($user && $user->id == $id))
		{
			...
			if ($this->db->affectedRows() === 1)
			{
				...
				$this->setMessage('Auth.activate_successful');
				return true;
			}
		}

		$this->triggerEvents(['post_activate', 'post_activate_unsuccessful']);
		$this->setError('Auth.activate_unsuccessful');
		return false;
	}
	...
	public function deactivate(int $id=0): bool
	{
		...
		else if ((new \App\Libraries\Auth())->loggedIn() && $this->user()->id == $id)
		{
			$this->setError('Auth.deactivate_current_user_unsuccessful');
			return false;
		}
		...
		if ($return)
		{
			$this->setMessage('Auth.deactivate_successful');
		}
		else
		{
			$this->setError('Auth.deactivate_unsuccessful');
	...
	public function resetPassword(string $identity, string $new)
	{
		...
		if ($return)
		{
			...
			$this->setMessage('Auth.password_change_successful');
		}
		else
		{
			$this->triggerEvents(['post_change_password', 'post_change_password_unsuccessful']);
	...
	public function identityCheck(string $identity=''): bool
	{
		...
		$builder = $this->db->table($this->tables['users']);
		return $builder->where('email', $identity)
	 ...
	 *
	 * @param string $identity As defined in Config/Auth
	 *
	...
	public function forgottenPassword(string $identity)
	{
		...
		$this->triggerEvents('extra_where');
		$this->db->table($this->tables['users'])->set($update)->where('username', $identity)->orWhere('email', $identity)->update();
	...
	public function getUserByForgottenPasswordCode(string $userCode)
	{
		...
		// Retrieve the user according to this selector
		//$user = $this->where('forgotten_password_selector', $token->selector)->user();
		$user = (new Users())->where('forgotten_password_selector', $token->selector)->get()->getFirstRow($this->config->userReturnType);
	...
	public function register(string $identity, string $password, string $email, array $additionalData=[], array $groups=[])
	{
		...
		if ($this->identityCheck($identity))
		{
			$this->setError('Auth.account_creation_duplicate_identity');
			return false;
		}
		else if (! $this->config->defaultGroup && empty($groups))
		{
			$this->setError('Auth.account_creation_missing_defaultGroup');
			return false;
		}
		...
		{
			$this->setError('Auth.account_creation_invalid_defaultGroup');
			return false;
		}
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		{
			$this->setError('Auth.login_unsuccessful');
			return false;
		}

		$this->triggerEvents('extra_where');
		$query = $this->db->table($this->tables['users'])
						  ->select('username, email, id, password, active, last_login')
		...
		if ($this->isMaxLoginAttemptsExceeded($identity))
		{
			...
			$this->setError('Auth.login_timeout');
		...
		if (isset($user))
		{
			if ($this->verifyPassword($password, $user->password, $identity))
			{
				...
				$this->setMessage('Auth.login_successful');
	...
	public function users($groups=null)
	{
		...
		return $this->response->getResult($this->config->userReturnType);
	}
	...
	public function user(int $id=0)
	{
		...
		return $this->response->getFirstRow($this->config->userReturnType);
	}
	...
	public function getUsersGroups(int $id=0)
	{
		...
		return $builder->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
					   ...
					   ->get()->getResult($this->config->userGroupsReturnType);
	}
	...
	public function inGroup($checkGroup, int $id=0, bool $checkAll=false): bool
	{
		...
		{
			$usersGroups = $this->getUsersGroups($id);
			$groupsArray = [];
	...
	public function groups()
	{
		...
		return $this->response->getResult($this->config->userGroupsReturnType);
	}
	...
	public function group(int $id=0)
	{
		...
		if ($id)
		{
			$builder->where($this->tables['groups'] . '.id', $id);
		}

		$builder->limit(1);
		$builder->orderBy('id', 'desc');

		return $builder->get()->getFirstRow($this->config->userGroupsReturnType);
	}
	...
	public function update(int $id, array $data): bool
	{
		...
		if (array_key_exists($this->identityColumn, $data) && $this->identityCheck($data[$this->identityColumn]) && $user->{$this->identityColumn} !== $data[$this->identityColumn])
		{
			...
			$this->setError('Auth.update_unsuccessful');
		...
		$this->setMessage('Auth.update_successful');
		return true;
	}
	...
	public function deleteUser(int $id): bool
	{
		...
		// delete user meta
		$this->db->table('usersmeta')->where('userid', $id)->delete();
		...
		if ($this->db->transStatus() === false)
		{
			...
			$this->setError('Auth.delete_unsuccessful');
			return false;
		}
	...
	public function createGroup(string $groupName='', string $groupDescription='', array $additionalData=[])
	{
		...
		if (! $groupName)
		{
			$this->setError('Auth.groupName_required');
			return false;
		}
		...
		if ($existingGroup !== 0)
		{
			$this->setError('Auth.group_already_exists');
			return false;
		}
		...
		// report success
		$this->setMessage('Auth.group_creation_successful');
	...
	public function updateGroup(int $groupId, string $groupName='', array $additionalData=[]): bool
	{
		...
		{
			$this->setError('Auth.groupName_admin_not_alter');
			return false;
		}
		...
		$this->setMessage('Auth.group_update_successful');
	...
	public function deleteGroup(int $groupId): bool
	{
		...
		if ($group->name === $this->config->adminGroup)
		{
			...
			$this->setError('Auth.group_delete_notallowed');
			return false;
		}
		...
		if ($this->db->transStatus() === false)
		{
			...
			$this->setError('Auth.group_delete_unsuccessful');
			return false;
		}
